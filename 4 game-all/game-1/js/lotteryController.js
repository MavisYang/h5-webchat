...
var isEmptyFlag = false;//表示抽奖是否都抽完，true:空
var controller = {
    init: function () {
    ...
    lottery: function () {
        $(".butt").click(function () {
            if (!isMoreFlag) {
                ...
                $(".partState").empty().text("每位宝妈只能抽取一次");
    ...
    getRule: function () {
        $(".lotteryRule").click(function () {
            ...
            $(".partState").empty().append("<ol style='text-align: left;margin-left:30px;font-size: 22px;line-height: 29px'><li>成为栗子宝妈俱乐部会员即可获得一次抽奖机会 </li>" +
...
var eventFunctions = {
    ...
    rotate: function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                else if(res.status == 10012){//谢谢参与 无可抽奖品
                    thanklag = true;
                }
        ...
        setTimeout(function () {
            ...
            if(liziFlag == true){    //只能抽取一次
                ...
                $(".partTitle").text("不要太贪心哦～")
                $(".partIMG").css({"background": "url('image/liziShort.png')no-repeat center center "})
                $(".partState").empty().text("每位宝妈只能抽取一次哦～");
                $(".negativeButton").val("查  看").attr("name", 'history.html');
                return;
            }
            if(thanklag == true){ //奖品已抽完
                modelFlag = true;
                para = 2;
                eventFunctions.handleModel();
                $(".partTitle").empty().text("很遗憾，奖品已抽完")
    ...
    setModel: function (offset, rewardReply, recordId, nickname, price, elecUrl,rewardId) {
        ...
        if (rewardReply === "e") { //电子券
            switch (rewardId){
                case 'e26':
                    partIMG = 'image/gift_e_26.png';
                    break;
                case 'e30':
                    partIMG = 'image/gift_e_30.png';
                    break;
                default:
                    break;
            }
            $(".partState").empty().text("亲爱的宝妈恭喜您获得"+nickname);
        ...
        if (rewardReply === "t") { //实物
            ...
            switch (rewardId){
                case 't27':
                    partIMG = 'image/gift_27.png';
                    break;
                case 't28':
                    partIMG = 'image/gift_28.png';
                    break;
                case 't29':
                    partIMG = 'image/gift_29.png';
                    break;
                case 't31':
                    partIMG = 'image/gift_31.png';
                    break;
                default:
                    break;
            }
            $(".partState").empty().text("亲爱的宝妈恭喜您获得此奖品");
            $(".partIMG").css({"background": "url(" + partIMG + ")no-repeat center center",'background-size': 'contain'})
